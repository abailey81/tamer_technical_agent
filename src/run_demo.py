...
def run_phase5(
    ...
    Generates 4 comprehensive output files:
        - {SYMBOL}_trade_note.html  (Institutional HTML)
        - {SYMBOL}_trade_note.pdf   (Publication PDF)
        - {SYMBOL}_trade_note.md    (Markdown)
        - {SYMBOL}_trade_note.json  (Structured Data)
    """
    print_section_header("PHASE 5: INSTITUTIONAL TRADE NOTE GENERATION (Claude Sonnet 4.5)")
    ...
    try:
        ...
        logger.info(f"API Key: {ANTHROPIC_API_KEY[:15]}...{ANTHROPIC_API_KEY[-8:]}")
        logger.info("=" * 70)
        ...
        # Count phases
        phases = []
        if phase1_output: phases.append("P1")
        if phase2_output: phases.append("P2")
        if phase3_output: phases.append("P3")
        if phase4_output: phases.append("P4")
        if phase4b_output: phases.append("P4B")
        logger.info(f"Phases: {', '.join(phases)}")
        
        logger.info("Calling Claude for comprehensive institutional analysis...")
        ...
        print(f"""
  ═══════════════════════════════════════════════════════════════════════════════
  Phase 5 Summary - INSTITUTIONAL TRADE NOTE GENERATION
  ═══════════════════════════════════════════════════════════════════════════════

    ╔══════════════════════════════════════════════════════════════════════════╗
    ║  RECOMMENDATION: {note.recommendation:<20}  CONVICTION: {note.conviction:<12} ║
    ║  CONFIDENCE:     {note.confidence_pct:<20.0f}%  RISK:       {note.risk_rating:<12} ║
    ╚══════════════════════════════════════════════════════════════════════════╝
    
    Score Analysis:
      Overall Score:   {note.overall_score:>8}/100
      Technical Score: {note.technical_score:>8}/100
      Momentum Score:  {note.momentum_score:>8}/100
      Trend Score:     {note.trend_score:>8}/100
      Risk Score:      {note.risk_score:>8}/100
    
    Price Levels:
      Current Price:   ${note.current_price:>12.2f}
      Target Price:    ${note.target_price:>12.2f} ({note.expected_return:+.1f}%)
      Entry Price:     ${note.trade_levels.entry:>12.2f}
      Stop Loss:       ${note.trade_levels.stop_loss:>12.2f}
      Target 1:        ${note.trade_levels.target_1:>12.2f}
      Target 2:        ${note.trade_levels.target_2:>12.2f}
      Target 3:        ${note.trade_levels.target_3:>12.2f}
    
    Position Sizing:
      Recommended:     {note.position_sizing.recommended_pct:>8.1f}%
      Maximum:         {note.position_sizing.max_pct:>8.1f}%
    
    Scenario Analysis:
      Bull Case:       ${note.scenarios[0].price_target if note.scenarios else 0:>12.2f} ({note.scenarios[0].probability*100 if note.scenarios else 0:.0f}%)
      Base Case:       ${note.scenarios[1].price_target if len(note.scenarios) > 1 else 0:>12.2f} ({note.scenarios[1].probability*100 if len(note.scenarios) > 1 else 0:.0f}%)
      Bear Case:       ${note.scenarios[2].price_target if len(note.scenarios) > 2 else 0:>12.2f} ({note.scenarios[2].probability*100 if len(note.scenarios) > 2 else 0:.0f}%)
    
    Generation:
      Model:           {note.model_used}
      Tokens Used:     {note.tokens_used:,}
      Time:            {note.generation_time_ms:.0f}ms
    
    ═══════════════════════════════════════════════════════════════════════════════
    OUTPUT FILES (4):
    ═══════════════════════════════════════════════════════════════════════════════
      • outputs/{symbol.upper()}_trade_note.html    (Institutional HTML Report)
      • outputs/{symbol.upper()}_trade_note.pdf     (Publication Quality PDF)
      • outputs/{symbol.upper()}_trade_note.md      (Markdown Documentation)
      • outputs/{symbol.upper()}_trade_note.json    (Structured JSON Data)
    ═══════════════════════════════════════════════════════════════════════════════
        """)
        ...
        logger.error(f"Phase 5 import failed: {e}")
        import traceback
        traceback.print_exc()
        return None
